...
export function CodexPanel({
  ...
  // 额度块渲染条件：至少一行有百分比数据（API 中转模式 rate_limits 为 null，不渲染）
  const hasRateRow = rate && (rate.primary_pct != null || rate.secondary_pct != null);

  return (
    <div className="flex-1 overflow-y-auto px-3.5 py-3 space-y-3">
      {/* 速率限制：5 小时窗口 + 周窗口（本机会话解析，不随时间范围变化） */}
      {hasRateRow && rate && (
        <div className="rounded-lg bg-white/25 border border-white/30 px-2.5 py-2 space-y-2">
          <div className="flex items-center justify-between">
            ...
            {rate.plan_type && (
              <span className="shrink-0 px-1.5 py-0.5 rounded bg-emerald-500/15 text-emerald-700 text-[9px] font-medium capitalize">
                {rate.plan_type}
              </span>
            )}
          </div>
