...
export const theme = {
  ...
  // 改完即时生效；窗口可自由拖拽调整大小（尺寸持久化），从未拖拽时高度
  // 随会话条数自适应，最多显示 5 条超出折叠。本卡只保留总开关/活跃档位/
  // 显示项——字体缩放与透明度入口在悬浮窗自身的设置面板内）=====
  "theme.hudTitle": "会话悬浮窗",
  "theme.hudHint":
    "独立透明置顶小窗：字体大小与窗口透明度在悬浮窗右上角的设置面板内调节。实时显示当前活跃 ZCode 会话的模型、状态（生成中/空闲）与累计 Token（Σ 总计 · ↑ 输入 · ↓ 输出 · ⟲ 缓存读 · × 请求 · 速度 t/s · TTFT，口径同注入版会话条），多会话并列、按最近活动排序；不依赖皮肤安装，可拖动移动位置、拖拽边缘自由调整大小。",
  ...
  "theme.hudShowTokensHint":
    "按注入版会话条口径显示每会话累计：Σ 总计 · ↑ 输入 · ↓ 输出 · ⟲ 缓存读 · × 请求 · 速度 t/s · TTFT 首字延迟，含列表下方按模型分组的速度行；关闭时底部「今日」合计行一并隐藏",
