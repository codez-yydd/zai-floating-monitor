 ...
 * - 自定义形象（第三阶段）：style 形如 custom:<id> 时经 get_custom_pet_asset
 *   命令读取资产（meta + 图集 dataUri）作为 customAsset 传给核心；
 *   资产读取失败（宠物被删除/元信息损坏）静默回退内建第一形象；
 *   参数事件每次抵达都重取资产（Rust 侧在导入替换后会重推参数事件，
 *   同 id 的资产对象变化触发核心重建画布，帧数据即时刷新）；
...
const main = async () => {
  ...
  // 形象与边长渲染宠物本体；读取失败按核心默认（cat / 档 3）渲染。
